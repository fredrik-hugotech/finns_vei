...
export async function createBikeTrip({ competitionId, club, helmet, distanceM = null, durationS = null, cells = [], path = [], tripToken = null } = {}) {
  ...
  logSupabase('bike_trip_logged', { competitionId, club: row.club, helmet: row.helmet, distanceM: row.distance_m, pathPoints: cleanPath.length });
...
export async function getCompetitionStats(id) {
  ...
  // Density heatmap: snap every route point to a fine ~35 m grid and count how
  // many trips pass through each cell. The weight makes popular roads glow.
  const HEAT_GRID = 35;
  ...
  for (const trip of rows) {
    ...
    for (const point of path) {
      const lng = Array.isArray(point) ? Number(point[0]) : Number(point?.lng);
      const lat = Array.isArray(point) ? Number(point[1]) : Number(point?.lat);
      const snapped = snapToGrid(lat, lng, HEAT_GRID);
      if (!snapped) continue;
      const key = `${snapped.lng},${snapped.lat}`;
      if (seen.has(key)) continue; // count each trip once per cell
      seen.add(key);
      const existing = heatCells.get(key);
      if (existing) existing.weight += 1;
      else heatCells.set(key, { lng: snapped.lng, lat: snapped.lat, weight: 1 });
    }
  }
  const features = Array.from(heatCells.values()).map((cell) => ({
    type: 'Feature',
