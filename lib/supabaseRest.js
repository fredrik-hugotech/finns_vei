...
export async function createBikeTrip({ competitionId, club, helmet, distanceM = null, durationS = null, cells = [], tripToken = null } = {}) {
  ...
  logSupabase('bike_trip_logged', { competitionId, club: row.club, helmet: row.helmet, distanceM: row.distance_m, cells: row.path_cells.length });
  return Array.isArray(rows) ? rows[0] : rows;
}

// Aggregate everything a public competition view needs: per-club leaderboard
// (trips, distance, helmet share) and an anonymous heatmap built from the union
// of all trips' snapped path cells — no individual route is ever exposed.
export async function getCompetitionStats(id) {
  ...
  const trips = await supabaseFetch(
    BIKE_TRIPS_TABLE,
    `?select=club,helmet,distance_m,duration_s,path_cells&competition_id=eq.${encodeURIComponent(id)}`,
    { method: 'GET' },
  );
  ...
  // Aggregate path cells across all trips into per-cell counts → heatmap points.
  const cellCounts = new Map();
  for (const trip of rows) {
    const cells = Array.isArray(trip.path_cells) ? trip.path_cells : [];
    for (const cell of cells) {
      const lng = Array.isArray(cell) ? Number(cell[0]) : Number(cell?.lng);
      const lat = Array.isArray(cell) ? Number(cell[1]) : Number(cell?.lat);
      ...
      else cellCounts.set(key, { lng, lat, weight: 1 });
    }
  }
  const features = Array.from(cellCounts.values()).map((cell) => ({
    type: 'Feature',
