...
export async function getReportThread(reportId) {
  ...
  const reports = await supabaseFetch(
    REPORTS_TABLE,
    `?id=eq.${encodeURIComponent(reportId)}&select=id,status,category,description,created_at,road_owner,road_authority,image_urls,support_count,public_status_note,public_status_updated_at,trello_card_id`,
    { method: 'GET' },
  );
