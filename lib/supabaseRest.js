...
// orphan it at its original public URL forever — despite the backoffice UI
// telling staff the delete is "permanent, cannot be undone".
export async function deleteCaseAttachment(id) {
  const attachment = await getCaseAttachment(id);
  if (attachment?.path) await deleteStorageObject(attachment.path);
