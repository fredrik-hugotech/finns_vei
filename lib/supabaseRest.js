...
export async function getCompetitionStats(id, { mode = null, includeGeojson = false } = {}) {
  ...
  const baseCols = 'club,helmet,distance_m,duration_s';
  const pathCol = includeGeojson ? ',path' : '';
  ...
  const [competition, trips] = await Promise.all([getCompetition(id), fetchTrips()]);
