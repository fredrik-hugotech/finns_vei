...
export async function listReportsForBackoffice({ limit = 300, idsOnly = false } = {}) {
  ...
  const select = idsOnly
    ? 'id'
    : 'id,category,status,description,lat,lng,created_at,status_updated_at,trello_card_id,public_status_note,bike_route_type,road_owner,speed_limit,due_date,assignee_email,support_count';
  const rows = await supabaseFetch(
    REPORTS_TABLE,
