...
function normalizePublicReportFeature(feature, row = {}) {
  ...
  const properties = {
    ...
    status_updated_at: safeProperties.status_updated_at ?? row?.status_updated_at ?? null,
...
export async function getPublicReportGeoJson() {
  ...
  try {
    const supportRows = await supabaseFetch(REPORTS_TABLE, '?select=id,support_count,road_reference,image_urls,public_status_note,public_status_updated_at,status_updated_at', { method: 'GET' });
    ...
    geojson.features = geojson.features.map((feature) => {
      ...
      const merged = {
        ...
        properties: {
          ...
          status_updated_at: feature.properties?.status_updated_at || context?.status_updated_at || null,
