...
function publicReportId(row, safeProperties = {}) {
  return safeProperties.id
    || safeProperties.report_id
    || safeProperties.reportId
    || row?.id
    || row?.report_id
    || row?.reportId
    || null;
...
function rowToFeature(row) {
  if (row?.type === 'Feature' && row.geometry) {
    const safeProperties = stripPrivateProperties(row.properties || {});
    return { ...row, properties: { ...safeProperties, id: publicReportId(row, safeProperties) } };
  }

  if (row?.feature?.type === 'Feature') {
    const safeProperties = stripPrivateProperties(row.feature.properties || {});
    return { ...row.feature, properties: { ...safeProperties, id: publicReportId(row, safeProperties) } };
  }

  if (row?.geojson?.type === 'Feature') {
    const safeProperties = stripPrivateProperties(row.geojson.properties || {});
    return { ...row.geojson, properties: { ...safeProperties, id: publicReportId(row, safeProperties) } };
  }
...
async function getReportSupportFingerprints(reportId) {
  const rows = await supabaseFetch(
    'report_supports',
    `?report_id=eq.${encodeURIComponent(reportId)}&select=id,support_token,ip_hash,user_agent_hash`,
    { method: 'GET' },
  );
  return Array.isArray(rows) ? rows : [];
}

async function countReportSupports(reportId) {
  const rows = await getReportSupportFingerprints(reportId);
  return rows.length;
}

function hasMatchingSupport(rows, { supportToken, ipHash, userAgentHash }) {
  return rows.some((row) => {
    const sameBrowserToken = supportToken && row.support_token === supportToken;
    const sameSoftFingerprint = ipHash && userAgentHash && row.ip_hash === ipHash && row.user_agent_hash === userAgentHash;
    return sameBrowserToken || sameSoftFingerprint;
  });
}
...
export async function createReportSupport({ reportId, supportToken, ipHash = null, userAgentHash = null }) {
  ...
  const existingSupports = await getReportSupportFingerprints(reportId);
  if (hasMatchingSupport(existingSupports, { supportToken, ipHash, userAgentHash })) {
    alreadySupported = true;
    logSupabase('report_support_duplicate', {
      reportId,
      hasSupportToken: Boolean(supportToken),
      hasSoftFingerprint: Boolean(ipHash && userAgentHash),
    });
  } else {
    try {
      ...
      logSupabase('report_support_inserted', { reportId, hasSupportToken: Boolean(supportToken), hasIpHash: Boolean(ipHash), hasUserAgentHash: Boolean(userAgentHash) });
    } catch (error) {
      if (error.status === 409) {
        alreadySupported = true;
        logSupabase('report_support_duplicate', { reportId, hasSupportToken: Boolean(supportToken) });
      } else {
        throw error;
      }
    }
  ...
  const updated = await updateReport(reportId, { support_count: supportCount });
  logSupabase('report_support_count_synced', { reportId, support_count: updated?.support_count ?? supportCount, alreadySupported });
