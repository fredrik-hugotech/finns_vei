...
export async function getPublicReportGeoJson() {
  ...
  try {
    const supportRows = await supabaseFetch(REPORTS_TABLE, '?select=id,support_count', { method: 'GET' });
    const supportById = new Map((supportRows || []).map((row) => [String(row.id), Number(row.support_count || 0)]));
    geojson.features = geojson.features.map((feature) => ({
      ...feature,
      properties: {
        ...feature.properties,
        support_count: supportById.get(String(feature.properties?.id)) ?? Number(feature.properties?.support_count || 0),
      },
    }));
  } catch (error) {
