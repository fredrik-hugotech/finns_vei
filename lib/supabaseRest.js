...
// Remove trips that did not come from a demo import (e.g. an older seed run)
// so the demo competition shows exactly the imported dataset.
export async function deleteNonImportedBikeTrips(competitionId) {
  await supabaseFetch(
    BIKE_TRIPS_TABLE,
    `?competition_id=eq.${encodeURIComponent(competitionId)}&or=(trip_token.is.null,trip_token.not.like.demo-import*)`,
    { method: 'DELETE', headers: { Prefer: 'return=minimal' } },
  );
}
