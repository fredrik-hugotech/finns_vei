...
export async function createReport(report) {
  let rows;
  try {
    rows = await supabaseFetch(REPORTS_TABLE, '', {
      method: 'POST',
      body: JSON.stringify(report),
    });
  } catch (error) {
    // `notify_on_status_change` may not be migrated yet on a given Supabase
    // project — retry without it rather than losing the whole report, same
    // resilience pattern as createBikeTrip's optional-column retry below.
    ...
    if (unknownColumn && Object.prototype.hasOwnProperty.call(report, 'notify_on_status_change')) {
      const { notify_on_status_change: _notify, ...rest } = report;
      logSupabase('report_insert_retry_without_notify_column', { category: report?.category });
      rows = await supabaseFetch(REPORTS_TABLE, '', {
        method: 'POST',
