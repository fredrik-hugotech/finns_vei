...
export async function getPublicReportGeoJson() {
  ...
  try {
    const supportRows = await supabaseFetch(REPORTS_TABLE, '?select=id,support_count,road_reference,image_urls,public_status_note,public_status_updated_at,speed_limit,road_owner,road_category', { method: 'GET' });
  ...
  try {
    const reportCards = await supabaseFetch(REPORTS_TABLE, '?select=id,trello_card_id', { method: 'GET' });
    const cardByReport = new Map((reportCards || []).map((row) => [String(row.id), row.trello_card_id || null]));
    const updateRows = await supabaseFetch('report_status_updates', '?select=trello_card_id,note,created_at,source&order=created_at.asc', { method: 'GET' });
    ...
    logSupabase('status_updates_merge_failed', { message: String(error?.message || '').slice(0, 240) });
  }
...
export async function createBikeTrip({ competitionId, club, helmet, origin, destination, tripToken = null } = {}) {
  // Snap the origin to a coarse grid here, server-side, so an exact start point
  // can never be persisted even if a client sends one.
  const snapped = origin ? snapToGrid(origin.lat, origin.lng, GRID_METERS) : null;

  const row = {
    competition_id: competitionId,
    club: String(club || '').trim() || null,
    helmet: Boolean(helmet),
    origin_lat: snapped ? snapped.lat : null,
    origin_lng: snapped ? snapped.lng : null,
    dest_lat: destination && Number.isFinite(Number(destination.lat)) ? Number(Number(destination.lat).toFixed(6)) : null,
    dest_lng: destination && Number.isFinite(Number(destination.lng)) ? Number(Number(destination.lng).toFixed(6)) : null,
    dest_name: destination && destination.name ? String(destination.name).trim() : null,
    trip_token: tripToken || null,
  };

  const rows = await supabaseFetch(BIKE_TRIPS_TABLE, '', {
    method: 'POST',
    headers: { Prefer: 'return=minimal' },
    body: JSON.stringify(row),
  });
  logSupabase('bike_trip_logged', { competitionId, club: row.club, helmet: row.helmet });
  return Array.isArray(rows) ? rows[0] : rows;
}

// Aggregate everything a public competition view needs: per-club leaderboard
// and a movement GeoJSON (snapped origin → venue lines + venue points).
export async function getCompetitionStats(id) {
  ...
  const trips = await supabaseFetch(
    BIKE_TRIPS_TABLE,
    `?select=club,helmet,origin_lat,origin_lng,dest_lat,dest_lng,dest_name&competition_id=eq.${encodeURIComponent(id)}`,
    { method: 'GET' },
  );
  ...
  for (const club of competition.clubs) {
    if (club?.name) byClub.set(club.name, { club: club.name, trips: 0, helmetTrips: 0 });
  }
  for (const trip of rows) {
    const name = trip.club || 'Ukjent';
    if (!byClub.has(name)) byClub.set(name, { club: name, trips: 0, helmetTrips: 0 });
  ...
  const leaderboard = Array.from(byClub.values())
    ...
    .sort((a, b) => b.trips - a.trips || a.club.localeCompare(b.club));

  const features = [];
  for (const trip of rows) {
    const hasOrigin = Number.isFinite(Number(trip.origin_lat)) && Number.isFinite(Number(trip.origin_lng));
    const hasDest = Number.isFinite(Number(trip.dest_lat)) && Number.isFinite(Number(trip.dest_lng));
    if (hasOrigin && hasDest) {
      features.push({
        type: 'Feature',
        properties: { kind: 'trip', helmet: Boolean(trip.helmet) },
        geometry: {
          type: 'LineString',
          coordinates: [[Number(trip.origin_lng), Number(trip.origin_lat)], [Number(trip.dest_lng), Number(trip.dest_lat)]],
        },
      });
    }
  }
  ...
  for (const trip of rows) {
    if (!Number.isFinite(Number(trip.dest_lat)) || !Number.isFinite(Number(trip.dest_lng))) continue;
    const key = `${trip.dest_lat},${trip.dest_lng}`;
    if (!venues.has(key)) {
      venues.set(key, {
        type: 'Feature',
        properties: { kind: 'venue', name: trip.dest_name || '', trips: 0 },
        geometry: { type: 'Point', coordinates: [Number(trip.dest_lng), Number(trip.dest_lat)] },
      });
    }
    venues.get(key).properties.trips += 1;
  }
  ...
  return {
    ...
    leaderboard,
    geojson: { type: 'FeatureCollection', features: [...features, ...venues.values()] },
  };
}
