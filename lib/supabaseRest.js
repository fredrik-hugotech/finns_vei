...
export async function listCaseAttachments(reportId, { publicOnly = false } = {}) {
  ...
  try {
    const rows = await supabaseFetch(CASE_ATTACHMENTS_TABLE, `?report_id=eq.${encodeURIComponent(reportId)}${filter}&select=id,url,path,filename,content_type,visibility,created_at&order=created_at.asc`, { method: 'GET' });
