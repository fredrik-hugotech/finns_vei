import { REPORT_IMAGE_BUCKET, RESOLUTION_IMAGE_MAX_COUNT, normalizeImageEntries } from './reportImages';
import { snapToGrid, GRID_METERS } from './geoPrivacy';
...
const REPORT_CONTEXT_SELECT = 'id,support_count,road_reference,image_urls,public_status_note,public_status_updated_at,speed_limit,road_owner,road_category,trello_card_id';
...
export async function getPublicReportGeoJson() {
  // The public features query and the three optional context/merge queries
  // don't depend on each other's results, so run them concurrently instead of
  // sequentially. Each merge query keeps its own independent error handling
  // (matching prior behavior: a failure in one merge doesn't block the others).
  const [rows, reportContextOutcome, reportSupportsOutcome, statusUpdatesOutcome] = await Promise.all([
    ...
    settleFetch(supabaseFetch('report_supports', '?select=report_id,note,category,created_at&order=created_at.asc', { method: 'GET' })),
    settleFetch(supabaseFetch('report_status_updates', '?select=trello_card_id,note,created_at,source&order=created_at.asc', { method: 'GET' })),
  ]);
...
// Detects PostgREST's "unknown column" error (42703, sometimes surfaced as
// PGRST204) — the same resilience idiom already used for
// report_supports.note/category: a column that hasn't been migrated onto this
// Supabase deployment yet degrades the feature instead of failing the request.
function isMissingColumnError(error) {
  const body = String(error?.body || error?.message || '');
  return body.includes('42703') || body.includes('PGRST204') || /column .* does not exist/i.test(body);
}

// Appends staff-uploaded "after" photos to `reports.resolution_image_urls`
// (before/after proof shown on the public case page once a case is fixed).
// Resilient by design: if the column hasn't been migrated onto this Supabase
// deployment yet, the caller (the upload endpoint) still gets the uploaded
// image URLs back — the storage upload already succeeded — just with
// `ok: false, columnMissing: true` so it can warn instead of 500ing.
export async function appendReportResolutionImages(id, newImages = []) {
  if (!id) return { ok: false, columnMissing: false, images: [] };

  let existing = [];
  try {
    const current = await getReportById(id);
    existing = normalizeImageEntries(current?.resolution_image_urls);
  } catch (_error) {
    existing = [];
  }

  // Keep the most recent RESOLUTION_IMAGE_MAX_COUNT photos across calls
  // (append, don't overwrite), mirroring REPORT_IMAGE_MAX_COUNT semantics.
  const merged = [...existing, ...normalizeImageEntries(newImages)].slice(-RESOLUTION_IMAGE_MAX_COUNT);

  try {
    const rows = await supabaseFetch(REPORTS_TABLE, `?id=eq.${encodeURIComponent(id)}`, {
      method: 'PATCH',
      body: JSON.stringify({ resolution_image_urls: merged }),
    });
    const updated = Array.isArray(rows) ? rows[0] : rows;
    logSupabase('report_resolution_images_updated', { reportId: id, count: merged.length });
    return { ok: true, columnMissing: false, images: merged, report: updated };
  } catch (error) {
    if (isMissingColumnError(error)) {
      logSupabase('report_resolution_images_column_missing', { reportId: id });
      return { ok: false, columnMissing: true, images: merged };
    }
    logSupabase('report_resolution_images_update_failed', { reportId: id, status: error?.status || null, message: String(error?.message || '').slice(0, 240) });
    throw error;
  }
}
...
export async function countStaff() {
  const rows = await supabaseFetch(STAFF_TABLE, '?select=id', { method: 'GET' });
...
export async function findNearbyOpenCases({ lat, lng, radiusM = 35 }) {
  ...
  const params = [
    ...
    `status=neq.${encodeURIComponent('Fullført')}`,
...
export async function createBikeTrip({ competitionId, club, helmet, distanceM = null, durationS = null, cells = [], path = [], routeType = null, mode = null, tripToken = null } = {}) {
  // `cells` are already clipped + snapped on the device. Re-snap server-side as a
  // defensive measure so nothing finer than the grid can ever be persisted, and
  // dedupe so the stored heatmap footprint stays compact.
  ...
  if (Array.isArray(cells)) {
    for (const cell of cells) {
      ...
      const snapped = snapToGrid(lat, lng, GRID_METERS);
      if (snapped) snappedCells.set(`${snapped.lng},${snapped.lat}`, [snapped.lng, snapped.lat]);
    }
  }

  // `path` is the ordered, end-clipped route used to draw the Strava-style line.
  // It is already trimmed of the home/destination ends on the device.
  const cleanPath = [];
  if (Array.isArray(path)) {
    for (const point of path) {
      ...
      if (Number.isFinite(lng) && Number.isFinite(lat)) cleanPath.push([Number(lng.toFixed(5)), Number(lat.toFixed(5))]);
    }
  }
  ...
  try {
    ...
    // `mode` column may not be migrated yet — persist the trip without it
    // rather than losing the ride entirely.
    const { mode: _drop, ...rest } = row;
    rows = await supabaseFetch(BIKE_TRIPS_TABLE, '', {
      method: 'POST',
      headers: { Prefer: 'return=minimal' },
  ...
  logSupabase('bike_trip_logged', { competitionId, club: row.club, helmet: row.helmet, distanceM: row.distance_m, mode: row.mode, pathPoints: cleanPath.length });
  return Array.isArray(rows) ? rows[0] : rows;
}
...
export async function getCompetitionStats(id, { mode = null } = {}) {
  ...
  try {
    trips = await supabaseFetch(
      BIKE_TRIPS_TABLE,
      `?select=club,helmet,distance_m,duration_s,path,mode&competition_id=eq.${encodeURIComponent(id)}`,
      { method: 'GET' },
    );
  } catch (_e) {
    trips = await supabaseFetch(
      BIKE_TRIPS_TABLE,
      `?select=club,helmet,distance_m,duration_s,path&competition_id=eq.${encodeURIComponent(id)}`,
      { method: 'GET' },
    );
  }
  ...
  for (const club of competition.clubs) {
    if (club?.name) byClub.set(club.name, { club: club.name, trips: 0, helmetTrips: 0, distanceM: 0 });
  }
  for (const trip of rows) {
    const name = trip.club || 'Ukjent';
    if (!byClub.has(name)) byClub.set(name, { club: name, trips: 0, helmetTrips: 0, distanceM: 0 });
  ...
  const leaderboard = Array.from(byClub.values())
    ...
    .sort((a, b) => {
      if (metric === 'distance' && b.distanceM !== a.distanceM) return b.distanceM - a.distanceM;
