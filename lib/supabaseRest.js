...
export async function deleteReport(id) {
  if (!id) return;
  try {
    const attachments = await supabaseFetch(CASE_ATTACHMENTS_TABLE, `?report_id=eq.${encodeURIComponent(id)}&select=id,path`, { method: 'GET' });
    if (Array.isArray(attachments) && attachments.length) {
      await Promise.all(attachments.map((a) => (a.path ? deleteStorageObject(a.path) : Promise.resolve())));
