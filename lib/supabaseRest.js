...
import { buildPaint, buildClubUsage } from './paintMap';
// Private bucket for internal-visibility case attachments — separate from the
// public report-images bucket used for report photos and public-visibility
// attachments. Files here are never reachable by a raw URL; every read goes
// through getSignedStorageUrl() for a short-lived, time-limited link. See the
// "Staff accounts and case attachments" section in README.md.
const CASE_ATTACHMENTS_INTERNAL_BUCKET = process.env.SUPABASE_STORAGE_BUCKET_CASE_ATTACHMENTS_INTERNAL || 'case-attachments-internal';
import { snapToGrid, GRID_METERS, clipAndSnapCells, clipPath, randomOffsetPoint } from './geoPrivacy';
import { classifyWeather } from './weather';
import { REPORT_STATUS } from './config';
import { buildHotspotOverview } from './hotspotAnalysis';
...
// Defensive cap for the public map's busiest endpoint (GET /api/reports) —
// flagged 2026-08-02 as fetching the entire reports history with no bound at
// all. At today's volume (well under a hundred rows) this changes nothing
// visible; it exists so growth can never turn this into an unbounded
// full-table scan on every map load. Deliberately NOT a product decision
// about hiding old "Fullført" cases (that's a separate, still-open
// question) — this only stops the query from being literally unbounded,
// ordered newest-first so if the cap is ever actually hit, what quietly
// drops off is the oldest history, not an arbitrary slice.
const PUBLIC_GEOJSON_MAX_ROWS = 5000;
...
export async function getPublicReportGeoJson() {
  ...
  // concurrent batch once those ids are known. Both queries share the same
  // newest-first order + cap so they line up on the same underlying set of
  // reports (report_public_geojson.feature already embeds created_at as a
  // JSON field, ordered via a ->> path expression; REPORTS_TABLE orders on
  // its own plain created_at column).
  const [rows, reportContextOutcome] = await Promise.all([
    supabaseFetch(PUBLIC_GEOJSON_VIEW, `?select=*&order=feature->properties->>created_at.desc&limit=${PUBLIC_GEOJSON_MAX_ROWS}`, { method: 'GET' }),
    settleFetch(supabaseFetch(REPORTS_TABLE, `?select=${REPORT_CONTEXT_SELECT}&order=created_at.desc&limit=${PUBLIC_GEOJSON_MAX_ROWS}`, { method: 'GET' })),
  ]);
...
// Short-lived signed URL for an object in a private bucket (only
// CASE_ATTACHMENTS_INTERNAL_BUCKET today). Returns null on any failure —
// callers should treat a missing signed URL as "not available right now"
// rather than fail the whole page, since this is generated fresh on every
// read and NVDB/network hiccups shouldn't break the case view.
// 1 hour default: long enough that a signed URL survives a staff member
// actively working a case page (viewing/scrolling attachments) without
// re-fetching, short enough that a leaked/logged link goes stale quickly.
export async function getSignedStorageUrl(path, bucket, expiresInSeconds = 3600) {
  if (!path || !bucket) return null;
  try {
    const response = await fetch(storageEndpoint(`/object/sign/${encodeURIComponent(bucket)}/${encodeStoragePath(path)}`), {
      method: 'POST',
    ...
    return storageEndpoint(data.signedURL);
  } catch (error) {
    logSupabase('signed_url_failed', { bucket, path, message: String(error?.message || '').slice(0, 200) });
    return null;
  }
}

// Copies an object from one bucket to another (Supabase Storage has no
// direct cross-bucket move, so this downloads then re-uploads) and returns
// the new path/bucket. Used when a case attachment's visibility is toggled
// between 'internal' (private bucket) and 'public' (public bucket) — the
// file physically moves, it isn't just relabeled, since a private-bucket
// object is never reachable by a plain URL regardless of any DB flag.
async function moveStorageObject(path, fromBucket, toBucket) {
  const downloadRes = await fetch(storageEndpoint(`/object/${encodeURIComponent(fromBucket)}/${encodeStoragePath(path)}`), {
    headers: { apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
  });
  if (!downloadRes.ok) {
    const text = await downloadRes.text();
    const error = new Error(`Storage download failed (${downloadRes.status})`);
    error.status = downloadRes.status;
    error.body = text;
    throw error;
  }
  const contentType = downloadRes.headers.get('content-type') || 'application/octet-stream';
  const buffer = Buffer.from(await downloadRes.arrayBuffer());
  const uploaded = await uploadReportImage({ path, buffer, contentType, bucket: toBucket });
  await deleteStorageObject(path, fromBucket);
  return uploaded;
}

// Best-effort: callers should not fail their own operation (e.g. deleting a
// DB row) just because the underlying storage object was already gone or the
// delete call itself failed — they should still remove the row so it doesn't
// linger forever, and log the failure for follow-up.
export async function deleteStorageObject(path, bucket = REPORT_IMAGE_BUCKET) {
  if (!path) return;
  try {
    const response = await fetch(storageEndpoint(`/object/${encodeURIComponent(bucket)}/${encodeStoragePath(path)}`), {
      method: 'DELETE',
    ...
    if (!response.ok && response.status !== 404) {
      const text = await response.text();
      logSupabase('storage_delete_failed', { bucket, path, status: response.status, body: text.slice(0, 400) });
      return;
    }
    logSupabase('storage_delete_completed', { bucket, path });
  } catch (error) {
    logSupabase('storage_delete_error', { bucket, path, message: error?.message });
  }
}
...
export async function getReportById(id, { select = '*' } = {}) {
  if (!id) return null;
  const rows = await supabaseFetch(REPORTS_TABLE, `?id=eq.${encodeURIComponent(id)}&select=${encodeURIComponent(select)}`, { method: 'GET' });
...
//
// Defensive cap, not pagination - mirrors the SUPPORT_LIMIT cap in
// getCaseSupport() below, so a long-open or high-visibility report can't
// serialize an unbounded, ever-growing row set into this public,
// unauthenticated, no-store endpoint on every anonymous request.
const REPORT_THREAD_SUPPORT_LIMIT = 500;
// Same reasoning applies to report_status_updates: a long-lived, heavily
// grouped case can accumulate many public Trello-comment updates over time.
// Capped to match getCaseTimeline()'s TIMELINE_LIMIT.
const REPORT_THREAD_UPDATES_LIMIT = 200;
...
export async function getReportThread(reportId) {
  ...
  const [supportsOutcome, statusUpdatesOutcome, attachmentsOutcome] = await Promise.all([
    settleFetch(supabaseFetch('report_supports', `?report_id=eq.${encodeURIComponent(reportId)}&select=note,category,created_at&order=created_at.desc&limit=${REPORT_THREAD_SUPPORT_LIMIT}`, { method: 'GET' })),
    report.trello_card_id
      ? settleFetch(supabaseFetch('report_status_updates', `?trello_card_id=eq.${encodeURIComponent(report.trello_card_id)}&select=note,created_at,source&order=created_at.desc&limit=${REPORT_THREAD_UPDATES_LIMIT}`, { method: 'GET' }))
  ...
  if (supportsOutcome.error) {
    ...
    // Fetched newest-first so the cap above keeps the freshest voices; restore
    // chronological (oldest-first) order for display.
    const supportRows = [...(supportsOutcome.data || [])].reverse();
    for (const row of supportRows) {
  ...
  if (statusUpdatesOutcome.error) {
    ...
    // Fetched newest-first so the cap above keeps the freshest updates; restore
    // chronological (oldest-first) order for display.
...
export async function getReportsSince(sinceIso) {
  ...
  const rows = await supabaseFetch(
    REPORTS_TABLE,
    `?select=${cols}&created_at=gte.${encodeURIComponent(sinceIso)}&order=created_at.desc`,
    { method: 'GET' },
  );
...
export async function getPublicReportById(id) {
  // Only public-facing fields are selected, so no contact-field stripping is
  // needed — unlike the old select=* version, which pulled the full ~35
  // column row (several jsonb fields) only to discard almost all of it.
  return getReportById(id, { select: 'id,category,description,status,image_urls,public_status_note' });
}
...
export async function getPublicReportsByIds(ids) {
  ...
  const rows = await supabaseFetch(REPORTS_TABLE, `?id=in.(${filter})&select=id,status,category,status_updated_at,public_status_updated_at`, { method: 'GET' });
  return Array.isArray(rows) ? rows : [];
}
...
export async function createReportSupport({ reportId, supportToken, ipHash = null, userAgentHash = null, note = null, category = null }) {
  ...
  let report = null;
  try {
    // Only an existence check (the caller never reads report fields beyond
    // id/support_count) — this is the highest-traffic public write path, so
    // avoid pulling every column (including contact_name/email/phone) just
    // to confirm the row exists.
    const rows = await supabaseFetch(REPORTS_TABLE, `?id=eq.${encodeURIComponent(reportId)}&select=id`, { method: 'GET' });
    report = Array.isArray(rows) ? rows[0] || null : rows;
  } catch (error) {
...
// is external and is not touched. case_attachments has no FK/cascade to
// reports, so its rows (and their Storage files) are cleaned up here
// best-effort before the report row goes — otherwise they'd become orphans
// pointing at a deleted case, unreachable from any UI but still publicly
// live at their storage URL. The report's own citizen-submitted image_urls
// live in the same bucket and are cleaned up the same way, for the same
// reason — "delete permanently" should not leave photos publicly reachable.
export async function deleteReport(id) {
  if (!id) return;
  // The two cleanup blocks touch independent tables/buckets (case_attachments
  // vs. the report's own image_urls) with no dependency on each other's result,
  // and each already contains its own error handling — so they can run
  // concurrently instead of sequentially, same as they'll both finish before
  // the final report-row delete below either way.
  await Promise.all([
    (async () => {
      try {
        const attachments = await supabaseFetch(CASE_ATTACHMENTS_TABLE, `?report_id=eq.${encodeURIComponent(id)}&select=id,path,visibility`, { method: 'GET' });
        if (Array.isArray(attachments) && attachments.length) {
          await Promise.all(attachments.map((a) => (a.path ? deleteStorageObject(a.path, bucketForVisibility(a.visibility)) : Promise.resolve())));
          await supabaseFetch(CASE_ATTACHMENTS_TABLE, `?report_id=eq.${encodeURIComponent(id)}`, { method: 'DELETE', headers: { Prefer: 'return=minimal' } });
        }
      } catch (error) {
        logSupabase('report_attachment_cleanup_failed', { reportId: id, message: error?.message });
      }
    })(),
    (async () => {
      try {
        const rows = await supabaseFetch(REPORTS_TABLE, `?id=eq.${encodeURIComponent(id)}&select=image_urls`, { method: 'GET' });
        const imageUrls = Array.isArray(rows) ? rows[0]?.image_urls : rows?.image_urls;
        if (Array.isArray(imageUrls) && imageUrls.length) {
          await Promise.all(imageUrls.map((img) => (img?.path ? deleteStorageObject(img.path) : Promise.resolve())));
        }
      } catch (error) {
        logSupabase('report_image_cleanup_failed', { reportId: id, message: error?.message });
      }
    })(),
  ]);
...
export async function getStaffSession(token) {
  // Embeds the related staff row via the staff_sessions.staff_id FK in the
  // same request, so getStaffFromRequest doesn't need a second round-trip
  // to look the staff member up by id on every backoffice request.
  const rows = await supabaseFetch(
    STAFF_SESSIONS_TABLE,
    `?select=staff_id,expires_at,staff:staff_id(id,email,name,role,active)&token=eq.${encodeURIComponent(token)}&limit=1`,
    { method: 'GET' },
  );
  return Array.isArray(rows) ? rows[0] || null : null;
}
...
// Invalidates every other active session for this staff member (e.g. on
// password change), keeping the session that made the request itself so the
// person who just changed their password isn't logged out on the spot.
export async function deleteOtherStaffSessions(staffId, keepToken) {
  await supabaseFetch(
    STAFF_SESSIONS_TABLE,
    `?staff_id=eq.${encodeURIComponent(staffId)}&token=neq.${encodeURIComponent(keepToken)}`,
...
// 'internal' attachments live in the private CASE_ATTACHMENTS_INTERNAL_BUCKET
// (never reachable by a plain URL); 'public' ones live in the same public
// REPORT_IMAGE_BUCKET report photos use. Every function below that touches
// Storage derives the bucket from visibility this way, rather than trusting a
// stored bucket name — there is no bucket column, visibility already
// determines it, and that keeps the two from ever disagreeing.
export function bucketForVisibility(visibility) {
  return visibility === 'public' ? REPORT_IMAGE_BUCKET : CASE_ATTACHMENTS_INTERNAL_BUCKET;
}
...
export async function createCaseAttachment({ reportId, url, path, contentType, filename, visibility = 'internal', size = null }) {
  ...
  const row = {
    report_id: reportId,
    // Internal attachments are never given a real, permanent URL — the
    // private bucket rejects it anyway. listCaseAttachments() signs one
    // fresh on every read instead.
    url: normalizedVisibility === 'public' ? url : null,
    path: path || null,
...
// Defensive cap, not pagination - same reasoning as REPORT_THREAD_SUPPORT_LIMIT/
// REPORT_THREAD_UPDATES_LIMIT above: a long-lived case can accumulate
// attachments indefinitely (no per-case count limit on upload), and this is
// hit from the public, unauthenticated report-thread endpoint too.
const CASE_ATTACHMENTS_LIMIT = 500;

export async function listCaseAttachments(reportId, { publicOnly = false } = {}) {
  const filter = publicOnly ? '&visibility=eq.public' : '';
  try {
    const rows = await supabaseFetch(CASE_ATTACHMENTS_TABLE, `?report_id=eq.${encodeURIComponent(reportId)}${filter}&select=id,url,path,filename,content_type,visibility,created_at&order=created_at.desc&limit=${CASE_ATTACHMENTS_LIMIT}`, { method: 'GET' });
    if (!Array.isArray(rows)) return [];
    // Fetched newest-first for the cap; reverse back to the chronological
    // (oldest-first) order callers expect.
    rows.reverse();
    // publicOnly callers only ever see visibility='public' rows (filtered
    // above), which already carry a real, permanent URL — no signing needed.
    if (publicOnly) return rows;
    // Otherwise (backoffice), sign a short-lived URL for every internal row
    // fresh on each read; independent lookups, so run them concurrently.
    return Promise.all(rows.map(async (row) => {
      if (row.visibility !== 'internal' || !row.path) return row;
      const signedUrl = await getSignedStorageUrl(row.path, CASE_ATTACHMENTS_INTERNAL_BUCKET);
...
// Deletes both the underlying Storage object and the DB row. case_attachments
// has no FK/cascade to reports, so leaving the storage file behind would
// orphan it forever — despite the backoffice UI telling staff the delete is
// "permanent, cannot be undone".
export async function deleteCaseAttachment(id) {
  const attachment = await getCaseAttachment(id);
  if (attachment?.path) await deleteStorageObject(attachment.path, bucketForVisibility(attachment.visibility));
  await supabaseFetch(CASE_ATTACHMENTS_TABLE, `?id=eq.${encodeURIComponent(id)}`, { method: 'DELETE', headers: { Prefer: 'return=minimal' } });
}

// Visibility isn't just a label — 'internal' and 'public' live in different
// buckets (see bucketForVisibility above), so toggling it has to physically
// move the file, not just flip a column. If the move fails, the DB row is
// left untouched (still pointing at the original bucket/visibility) rather
// than end up claiming a visibility the file was never actually moved to.
export async function setCaseAttachmentVisibility(id, visibility) {
  const nextVisibility = visibility === 'public' ? 'public' : 'internal';
  const attachment = await getCaseAttachment(id);
  if (!attachment) return;
  if (attachment.visibility === nextVisibility) return; // no-op, nothing to move
  if (!attachment.path) {
    // No underlying file (shouldn't normally happen) — just flip the label.
    await supabaseFetch(CASE_ATTACHMENTS_TABLE, `?id=eq.${encodeURIComponent(id)}`, {
      method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ visibility: nextVisibility }),
    });
    return;
  }
  const fromBucket = bucketForVisibility(attachment.visibility);
  const toBucket = bucketForVisibility(nextVisibility);
  const moved = await moveStorageObject(attachment.path, fromBucket, toBucket);
  await supabaseFetch(CASE_ATTACHMENTS_TABLE, `?id=eq.${encodeURIComponent(id)}`, {
    method: 'PATCH',
    headers: { Prefer: 'return=minimal' },
    // Same path string in both buckets (moveStorageObject re-uses it), but
    // the stored `url` only makes sense for the public bucket — an internal
    // row's url is always signed fresh on read (see listCaseAttachments).
    body: JSON.stringify({ visibility: nextVisibility, path: moved.path, url: nextVisibility === 'public' ? moved.url : null }),
  });
}

// Internal: list recent reports for the staff dashboard (includes Trello id).
export async function listReportsForBackoffice({ limit = 300, idsOnly = false } = {}) {
  // idsOnly: same order/limit as the full list, but selecting a single
  // column — for callers that only need the id sequence (e.g. the case
  // workspace's prev/next navigation), not the other 14 columns.
  const select = idsOnly
    ? 'id'
    : 'id,category,status,description,lat,lng,created_at,status_updated_at,trello_card_id,public_status_note,bike_route_type,road_owner,road_category,speed_limit,due_date,assignee_email,support_count,reporter_type';
  const rows = await supabaseFetch(
    REPORTS_TABLE,
...
// Internal: bounded set of reports for the staff stats dashboard
// (pages/backoffice/statistikk.js). Selects only the scalar fields needed
// for aggregation (volume over time, category/status counts, resolution
// time, NVDB road-owner/road-category breakdown) — no contact info. Other
// backoffice lists cap much lower (listReportsForBackoffice: 300) because
// they only cover open/recent cases; stats look across more history, so the
// ceiling is higher but still capped rather than pulling every row ever.
export async function listReportsForStats({ since, limit = 2000 } = {}) {
  const select = 'id,category,status,created_at,status_updated_at,road_owner,road_category,support_count';
  const params = [`select=${select}`, 'order=created_at.desc', `limit=${limit}`];
  if (since) params.push(`created_at=gte.${encodeURIComponent(since)}`);
  const rows = await supabaseFetch(REPORTS_TABLE, `?${params.join('&')}`, { method: 'GET' });
  return Array.isArray(rows) ? rows : [];
}
...
export async function getCaseSupport(reportId) {
  ...
  const voices = [];
  try {
    // Defensive cap, not pagination - already newest-first, so a hit cap
    // keeps the freshest citizen voices instead of silently favouring old ones.
    const SUPPORT_LIMIT = 500;
    const rows = await supabaseFetch(
      'report_supports',
      `?report_id=eq.${encodeURIComponent(reportId)}&select=note,category,created_at&order=created_at.desc&limit=${SUPPORT_LIMIT}`,
      { method: 'GET' },
    );
...
export async function countReportsByTrelloCard(trelloCardId) {
  ...
  try {
    // Exact count via HEAD request - no need to transfer full rows just to count them.
    return await supabaseCount(REPORTS_TABLE, `?trello_card_id=eq.${encodeURIComponent(trelloCardId)}&select=id`);
  } catch (error) {
    return 0;
  }
}
...
export async function getCaseTimeline(trelloCardId) {
  ...
  try {
    // Defensive cap (status updates are far rarer than supports, hence the
    // smaller cap). Fetch newest-first so a hit cap keeps the freshest
    // updates, then reverse back to the oldest-first order callers expect.
    const TIMELINE_LIMIT = 200;
    const rows = await supabaseFetch('report_status_updates', `?trello_card_id=eq.${encodeURIComponent(trelloCardId)}&select=note,created_at,source&order=created_at.desc&limit=${TIMELINE_LIMIT}`, { method: 'GET' });
    return (rows || [])
      .slice()
      .reverse()
      .map((row) => ({ note: typeof row.note === 'string' ? row.note.trim() : '', created_at: row.created_at || null, source: row.source || 'trello_comment' }))
...
export async function approveReportAiPublicStatus(id) {
  const report = await getReportById(id, { select: 'id,ai_public_status_suggestion' });
...
// 'trips' (flest turer) | 'distance' (flest km) | 'paint' («Mal kartet»: flest malte veibiter)
function normalizeMetric(value) {
  return value === 'distance' || value === 'paint' ? value : 'trips';
}
...
export async function listCompetitions({ activeOnly = false } = {}) {
  ...
  const rows = await supabaseFetch(
    COMPETITIONS_TABLE,
    `?select=*${filter}&order=created_at.desc&limit=200`,
    { method: 'GET' },
  );
...
export async function createBikeTrip({ competitionId, club, helmet, distanceM = null, durationS = null, cells = [], path = [], routeType = null, mode = null, weather = null, tripToken = null } = {}) {
  ...
  // hostile client — and never anchor that re-clip on the client's claimed
  // start point directly. A previous version did exactly that
  // (homeRef = rawPathPoints[0]), which meant the clip boundary was always
  // an exact, client-computable radius around whatever the client declared
  // as index 0 — a forged request could place the true start just outside
  // that known radius and have it survive uncensored. Instead, the server
  // generates its own fresh random offset around the declared start and
  // clips around THAT — a boundary the client can neither predict nor place
  // points safely outside of, and the server itself never reads or stores
  // the client's claimed start as a meaningful "this is home" value either.
  const rawPathPoints = [];
  ...
  const declaredStart = rawPathPoints[0] || rawCellPoints[0] || null;
  const homeRef = declaredStart ? randomOffsetPoint(declaredStart) : null;

  const cleanPath = clipPath(rawPathPoints, { homeRef });
  const snappedCells = new Map();
  if (homeRef) {
    for (const [lng, lat] of clipAndSnapCells(rawCellPoints, { gridMeters: GRID_METERS, homeRef })) {
...
export const CLUB_COLORS = ['#0b5d4d', '#e8590c', '#1d4ed8', '#7c3aed', '#c2185b', '#15803d', '#b45309', '#0e7490', '#be123c', '#4d7c0f', '#6d28d9', '#0369a1', '#a16207', '#db2777', '#047857', '#9f1239'];
...
// Per-trip route lines for a competition, oldest first — used ONLY by the
// public demo page (pages/api/demo/spor.js), which refuses to serve anything
// but competitions flagged "demo" in their name/description. Real children's
// routes are never exposed per trip anywhere; see README "Privacy by design".
export async function listCompetitionTripRoutes(id, { limit = 2000 } = {}) {
  const rows = await supabaseFetch(
    BIKE_TRIPS_TABLE,
    `?select=club,helmet,mode,route_type,distance_m,duration_s,weather_bonus,created_at,path&competition_id=eq.${encodeURIComponent(id)}&order=created_at.asc&limit=${limit}`,
    { method: 'GET' },
  );
  const out = [];
...
export async function getCompetitionStats(id, { mode = null, includeGeojson = false } = {}) {
  ...
  // `path` (the full clipped GPS route JSON per trip) is only read below when
  // includeGeojson is true — every current public/OG caller passes false, so
  // the leaderboard-only path was fetching and discarding every trip's route
  // geometry on every request. Drop it from the select when it won't be used.
  const baseCols = 'club,helmet,distance_m,duration_s,created_at';
  // «Mal kartet» needs every route to work out which club owns each road.
  const preCompetition = await getCompetition(id);
  const wantsPaint = preCompetition?.metric === 'paint';
  const pathCol = includeGeojson || wantsPaint ? ',path' : '';
  const fetchTrips = async () => {
    try {
      return await supabaseFetch(
        BIKE_TRIPS_TABLE,
        `?select=${baseCols}${pathCol},mode,weather_bonus&competition_id=eq.${encodeURIComponent(id)}&limit=${TRIPS_LIMIT}`,
        { method: 'GET' },
      );
    } catch (_e) {
      return supabaseFetch(
        BIKE_TRIPS_TABLE,
        `?select=${baseCols}${pathCol}&competition_id=eq.${encodeURIComponent(id)}&limit=${TRIPS_LIMIT}`,
        { method: 'GET' },
      );
    }
  };
  // `competition` and `trips` are independent reads (trips only needs `id`) -
  // fetch them concurrently instead of waiting on `getCompetition` first.
  if (!preCompetition) return null;
  const competition = preCompetition;
  ...
  const metric = competition.metric === 'paint' ? 'paint' : (competition.metric === 'distance' ? 'distance' : 'trips');
  // «Mal kartet»: which club owns each ~30 m road edge (>= MIN_TRIPS trips
  // from the club, ties to the first). Club-level aggregate only.
  const paint = metric === 'paint'
    ? buildPaint(rows.map((t) => ({ club: t.club || 'Ukjent', path: Array.isArray(t.path) ? t.path : [], createdAt: t.created_at })), { colorFor })
    : null;
  const edgesByClub = new Map((paint?.leaderboard || []).map((r) => [r.club, r]));
  const leaderboard = Array.from(byClub.values())
    .map((entry) => ({
      ...
      edges: edgesByClub.get(entry.club)?.edges || 0,
      paintPct: edgesByClub.get(entry.club)?.percent || 0,
    }))
    .sort((a, b) => {
      // Rank by the real metric — living somewhere with nice weather is never
      // a disadvantage.
      if (metric === 'paint') { if (b.edges !== a.edges) return b.edges - a.edges; }
      else if (metric === 'distance') { if (b.distanceM !== a.distanceM) return b.distanceM - a.distanceM; }
  ...
  // Only the internal admin heatmap (backoffice/tetthet) needs this — public
  // callers (competitions/[id].js, the certificate OG image) already discard
  // `geojson` from the result, so skip the O(trips × path length) work for them.
  let features = [];
  if (includeGeojson) {
    const SEG_GRID = 25;
    ...
    features = Array.from(segments.values()).map((seg) => ({
      type: 'Feature',
  ...
  return {
    ...
    // Club-coloured painted roads for «Mal kartet» (null for other metrics).
    paint: paint ? paint.geojson : null,
    paintedEdges: paint ? paint.paintedEdges : 0,
    // Internal (backoffice) only: which clubs use which roads, parallel strands.
    clubUsage: includeGeojson
      ? buildClubUsage(rows.map((t) => ({ club: t.club || 'Ukjent', path: Array.isArray(t.path) ? t.path : [] })), { minTrips: 1, colorFor })
      : null,
  };
}
...
// ---------------------------------------------------------------------------
// Recurring hotspot / seasonal-pattern overview (see lib/hotspotAnalysis.js
// for the clustering + ranking logic). Unlike listHotCases above, this reads
// the FULL report history — any status, any age — because a chronic spot's
// pattern only shows up across time, not in a snapshot of what's open today.
// Read-only: no writes, no schema changes.
// ---------------------------------------------------------------------------
const HOTSPOT_REPORT_SELECT = 'id,lat,lng,created_at,category,status';

export async function listRecurringHotspots({ limit = 5000, radiusM, minPeriods } = {}) {
  const rows = await supabaseFetch(
    REPORTS_TABLE,
    `?select=${HOTSPOT_REPORT_SELECT}&order=created_at.asc&limit=${limit}`,
    { method: 'GET' },
  );
  const reports = Array.isArray(rows) ? rows : [];
  const truncated = reports.length >= limit;
  if (!reports.length) return { hotspots: [], truncated };
  return {
    hotspots: buildHotspotOverview(reports, {
      ...(radiusM ? { radiusM } : {}),
...
// ---------------------------------------------------------------------------
// Demo import helpers (pages/api/demo/import.js). Only ever used against
// competitions flagged "demo"; the endpoint enforces that.
export async function listBikeTripTokens(competitionId) {
  const rows = await supabaseFetch(
    BIKE_TRIPS_TABLE,
    `?select=trip_token&competition_id=eq.${encodeURIComponent(competitionId)}&trip_token=not.is.null&limit=5000`,
    { method: 'GET' },
  );
  return new Set((Array.isArray(rows) ? rows : []).map((r) => r.trip_token).filter(Boolean));
}

export async function insertBikeTripRows(rows) {
  if (!rows.length) return;
  await supabaseFetch(BIKE_TRIPS_TABLE, '', {
    method: 'POST',
    headers: { Prefer: 'return=minimal' },
...
// Remove every trip in a demo competition that is not part of the current
// dataset (`keepPrefix` = the trip_token prefix of the generation to keep).
export async function deleteNonImportedBikeTrips(competitionId, keepPrefix = 'demo-import') {
  const like = encodeURIComponent(`${keepPrefix}*`);
  await supabaseFetch(
    BIKE_TRIPS_TABLE,
    `?competition_id=eq.${encodeURIComponent(competitionId)}&or=(trip_token.is.null,trip_token.not.like.${like})`,
    { method: 'DELETE', headers: { Prefer: 'return=minimal' } },
  );
}

export async function setCompetitionClubs(competitionId, clubs) {
  await supabaseFetch(
    COMPETITIONS_TABLE,
    `?id=eq.${encodeURIComponent(competitionId)}`,
    { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ clubs: normalizeClubs(clubs) }) },
  );
}

// Earlier routes of one club in a competition — for «Mal kartet», to tell a
// child how many road edges their trip just painted. Club aggregate only.
export async function listClubTripPaths(competitionId, club, { limit = 2000 } = {}) {
  const rows = await supabaseFetch(
    BIKE_TRIPS_TABLE,
    `?select=path&competition_id=eq.${encodeURIComponent(competitionId)}&club=eq.${encodeURIComponent(club)}&order=created_at.desc&limit=${limit}`,
    { method: 'GET' },
  );
  return (Array.isArray(rows) ? rows : []).map((r) => ({ path: Array.isArray(r.path) ? r.path : [] })).filter((r) => r.path.length > 1);
}
