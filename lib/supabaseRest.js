...
export async function createBikeTrip({ competitionId, club, helmet, distanceM = null, durationS = null, cells = [], path = [], routeType = null, mode = null, weather = null, tripToken = null } = {}) {
  ...
  const row = {
    ...
    precip_mm: Number.isFinite(Number(w?.precipMm)) ? Number(w.precipMm) : null,
    temp_c: Number.isFinite(Number(w?.tempC)) ? Number(w.tempC) : null,
    weather_bonus: Boolean(w?.bonus),
  ...
  try {
    ...
    // `mode`/weather columns may not be migrated yet — persist the trip without
    // the optional columns rather than losing the ride entirely.
    const { mode: _m, weather_symbol: _ws, precip_mm: _p, temp_c: _t, weather_bonus: _wb, ...rest } = row;
    rows = await supabaseFetch(BIKE_TRIPS_TABLE, '', {
      method: 'POST',
      headers: { Prefer: 'return=minimal' },
...
export async function getCompetitionStats(id, { mode = null } = {}) {
  ...
  const leaderboard = Array.from(byClub.values())
    .map((entry) => ({
      ...
      // Bad-weather trips count double toward the "flest turer" score.
      score: entry.trips + entry.bonusTrips,
    }))
    .sort((a, b) => {
