...
export async function getReportThread(reportId) {
  ...
  const voices = [];
  try {
    const supports = await supabaseFetch('report_supports', `?report_id=eq.${encodeURIComponent(reportId)}&select=note,category,created_at&order=created_at.asc`, { method: 'GET' });
    ...
    logSupabase('thread_supports_failed', { reportId, message: String(error?.message || '').slice(0, 200) });
  }

  let updates = [];
  if (report.trello_card_id) {
    try {
      const rows = await supabaseFetch('report_status_updates', `?trello_card_id=eq.${encodeURIComponent(report.trello_card_id)}&select=note,created_at,source&order=created_at.asc`, { method: 'GET' });
      updates = (rows || [])
      ...
      logSupabase('thread_updates_failed', { reportId, message: String(error?.message || '').slice(0, 200) });
    }
  }

  let attachments = [];
  try {
    attachments = (await listCaseAttachments(reportId, { publicOnly: true })).map((a) => ({ url: a.url, content_type: a.content_type || null }));
...
export async function listReportsForBackoffice({ limit = 300 } = {}) {
  const select = 'id,category,status,description,lat,lng,created_at,trello_card_id,public_status_note,bike_route_type,road_owner,speed_limit,due_date,assignee_email,support_count';
