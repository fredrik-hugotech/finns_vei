...
export function vegdataLines(data = {}) {
  return [
    ...
    `Veikategori: ${roadCategoryLabel(data.road_category) || 'Ikke funnet'}`,
    `Vegnummer: ${data.road_number || 'Ikke funnet'}`,
    `Veieier/vegmyndighet: ${data.road_authority || data.road_owner || 'Ikke funnet'}`,
  ...
  try {
    ...
    const patch = vegdata && hasUsefulVegdata(vegdata)
      ? {
          ...
          nvdb_note: vegdata.nvdb_note || null,
        }
      : {
