...
import { countReportsByTrelloCard, findNearbyOpenCases, setReportCaseId, updateReport } from './supabaseRest';
import { addTrelloCardAttachment, addTrelloCardComment, createTrelloCard, getNewReportListId, hasTrelloConfig, updateTrelloCardDescription, updateTrelloCardName } from './trello';

const CASE_GROUP_RADIUS_M = Number(process.env.CASE_GROUP_RADIUS_M || 35);

export function siteBaseUrl() {
  const explicit = process.env.PUBLIC_BASE_URL || process.env.NEXT_PUBLIC_SITE_URL;
...
// A report can have real nearby accident history even when no NVDB road
// match was found - that's still useful context and shouldn't be reported as
// "no relevant vegdata found".
function hasUsefulAccidentData(vegdata = {}) {
  const summary = vegdata.accident_summary || {};
  const count = Number(vegdata.accident_count ?? summary.count ?? 0);
...
export function buildBaseTrelloDescription(report, { includeImages = true } = {}) {
  ...
  const caseId = report.case_id || report.id;
  const caseLink = baseUrl && caseId ? `**Finns Fairway-sak:** [Åpne saken](${baseUrl}/sak/${caseId})` : null;
...
export async function createMissingTrelloCardBestEffort(report) {
  ...
  try {
    ...
    // Two independent writes to different columns of the same report row
    // (setReportCaseId already swallows its own errors internally and never
    // rejects), so they can run concurrently instead of sequentially — same
    // pattern already used for independent writes elsewhere in this repo.
    const [updatedReport] = await Promise.all([
      ...
      setReportCaseId(report.id, report.id),
    ]);
    ...
    const patchedNote = `Trello-feil: ${errorSummary(error)}`;
    try {
      const updatedReport = await updateReport(report.id, { nvdb_note: patchedNote });
      // Return the patched report (not the stale input) so downstream steps
      // (e.g. enrichReportBestEffort) build their nvdb_note on top of this
      // failure note instead of overwriting it with a note computed from the
      // pre-failure report.
      return {
        report: { ...report, ...updatedReport, nvdb_note: patchedNote },
        warning: 'Meldingen er lagret, men Trello-kort ble ikke laget.',
      };
    } catch (updateError) {
...
export async function enrichReportBestEffort(report, { updateTrelloCard = true } = {}) {
  ...
  try {
    ...
    if (updateTrelloCard && trelloCardId) {
      try {
        // Two independent Trello REST calls (description update vs. image
        // attachment) with no data dependency between them — run concurrently.
        await Promise.all([
          updateTrelloCardDescription(trelloCardId, buildEnrichedTrelloDescription({ ...report, ...updatedReport }, patch)),
          attachImagesToTrelloBestEffort(trelloCardId, { ...report, ...updatedReport }),
        ]);
      } catch (error) {
...
async function tryGroupIntoExistingCase(report) {
  ...
  const anchor = candidates
    ...
    .map((row) => ({ ...row, dist: haversineMeters(lat, lng, Number(row.lat), Number(row.lng)) }))
  ...
  // Same independent-writes-to-the-same-row situation as
  // createMissingTrelloCardBestEffort above — run concurrently.
  const [updated] = await Promise.all([
    updateReport(report.id, {
      trello_card_id: anchor.trello_card_id,
      trello_list_id: anchor.trello_list_id,
    }),
    setReportCaseId(report.id, caseId),
  ]);
  ...
  try {
    ...
    // Two independent Trello REST calls (add-comment vs. image attachment)
    // with no data dependency between them — run concurrently.
    await Promise.all([
      addTrelloCardComment(anchor.trello_card_id, lines.join('\n')),
      attachImagesToTrelloBestEffort(anchor.trello_card_id, report),
    ]);
  } catch (error) {
  ...
  try {
    const count = await countReportsByTrelloCard(anchor.trello_card_id);
    if (count > 1) {
      await updateTrelloCardName(anchor.trello_card_id, `Sak: ${anchor.category} · ${count} meldinger`);
    }
  } catch (error) {
    logWorkflow('case_rename_failed', { reportId: report.id, cardId: anchor.trello_card_id, error: errorSummary(error) });
  }
