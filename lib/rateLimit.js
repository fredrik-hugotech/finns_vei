...
/**
 * Extract the client IP. Prefers `x-real-ip`, which Vercel's edge network
 * sets to the actual connecting client IP and which callers cannot set
 * themselves. Falls back to the LAST entry of `x-forwarded-for` (the hop
 * closest to Vercel's own proxy, i.e. the one Vercel itself appended -
 * NOT the first entry, which is attacker-controlled and must never be
 * trusted for rate limiting), and finally to the socket address.
 */
