...
export function addMyTrip({ distanceM, mode, routeType, weather, helmet = null, club = null }) {
  if (typeof window === 'undefined') return;
  try {
    ...
    const entry = {
      ...
      helmet: helmet === null ? null : Boolean(helmet),
      club: club || null,
...
// The club the child last logged a trip for — used to highlight "din klubb"
// in the standings. Local only, like everything else here.
const CLUB_KEY = 'finns-vei-my-club';
export function setMyClub(name) {
  if (typeof window === 'undefined' || !name) return;
  try { window.localStorage.setItem(CLUB_KEY, String(name)); } catch (_e) { /* best effort */ }
}
export function getMyClub() {
  if (typeof window === 'undefined') return '';
  try { return window.localStorage.getItem(CLUB_KEY) || ''; } catch (_e) { return ''; }
}
