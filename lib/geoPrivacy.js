...
// Ordered, end-clipped route for drawing a Strava-style line. We remove the
// segments within ~150 m of the start and end (a "privacy zone" like Strava's),
// so the home and exact destination are never drawn, but the on-road middle is
// kept at full resolution. Returns ordered [lng, lat] pairs.
export function clipPath(points = [], { clipMeters = CLIP_METERS } = {}) {
  ...
  for (const point of points) {
    if (!point) continue;
    if (distanceMeters(point, start) <= clipMeters) continue;
    if (distanceMeters(point, end) <= clipMeters) continue;
