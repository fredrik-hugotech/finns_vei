...
export function classifyRoadAuthority(report = {}) {
  ...
  // authorityKind exposes the full classification ('state'|'county'|
  // 'municipal'|'private'|null) — unlike authorityType below (only ever
  // 'state'|'county', since municipal/private cases have nothing to refer
  // anywhere) — so other callers (e.g. the backoffice case-list "vegeier"
  // filter) can reuse this same classification logic instead of keeping
  // their own copy.
  if (kind === 'state') {
...
/**
 * Build a plain-text email draft (used both for the mailto: link body and
 * the copy-to-clipboard fallback) summarizing a case. Pure formatting —
 ...
 *
 * Originally built only for non-municipal cases (a genuine referral to
 * Statens vegvesen/fylkeskommunen — classification.authorityName set).
 * Also usable for a municipal/private/unknown-road case, where there is no
 * "wrong authority" to refer to — the intro/subject drop the referral
 * framing and read as a plain internal case summary instead, for staff who
 * want a ready-made email to forward internally (e.g. to their own
...
export function buildReferralDraft(report = {}, classification) {
  const authorityName = classification?.authorityName || null;
  ...
  const subjectParts = [
    ...
    authorityName ? `– videresendes til ${authorityName}` : null,
  ].filter(Boolean);
  ...
  const intro = authorityName
    ...
    : 'Vi har mottatt en trafikksikkerhetsmelding fra en innbygger. Se detaljer under.';
