...
//
// Client IP is taken from `x-real-ip` first (set by Vercel's edge network to
// the actual connecting client IP and not client-settable), falling back to
// the LAST entry of `x-forwarded-for` (the hop Vercel's own proxy appended,
// not the first entry, which is attacker-controlled and must never be
// trusted for rate limiting). There is no socket to fall back to in the edge
// runtime, so an absent/empty header simply yields ''.
//
