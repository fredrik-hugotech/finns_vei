...
// bbox cache keys are near-unique floating point strings (from map pan/zoom),
// so this cache would otherwise grow unbounded on a long-warm serverless
// instance. Cap it the same way lib/rateLimit.js caps its buckets Map.
...
function paginationBudgetMs() {
  const configured = Number(process.env.NVDB_PAGINATION_BUDGET_MS);
...
function cacheSet(key, value) {
  if (cache.size >= MAX_CACHE_ENTRIES) {
    // Cheap safety valve: drop the oldest-looking entry rather than let
...
// Follows an already-resolved NVDB "next page" link (metadata.neste.href) —
// unlike nvdbGet() this never falls back across base URLs, since the href
// came from whichever base already answered successfully.
async function nvdbGetUrl(href, cacheKey) {
  const cached = cache.get(cacheKey);
  if (cached && cached.expires > Date.now()) return cached.value;
  const value = await fetchJsonWithRetry(new URL(href));
  cacheSet(cacheKey, value);
  return value;
}

function parseWkt(wkt = '') {
  const normalized = String(wkt).trim().replace(/^SRID=\d+;/i, '');
  const type = normalized.split(/[\s(]/)[0]?.toUpperCase();
  const body = normalized.replace(/^[A-Z]+(?:\s+(?:ZM|M|Z))?\s*/i, '').trim();
...
function normalizeLonLatPair(lng, lat) {
  ...
  if (Math.abs(lon) <= 180 && Math.abs(latitude) <= 90) {
    if (lon >= 55 && lon <= 72 && latitude >= 4 && latitude <= 32) {
      return [latitude, lon];
    }
    return [lon, latitude];
  }
...
function coordinateInBbox(coordinate, bboxBounds) {
  if (!coordinate || !bboxBounds) return true;
  const [lng, lat] = coordinate;
  return lng >= bboxBounds.minLng
    && lng <= bboxBounds.maxLng
    && lat >= bboxBounds.minLat
    && lat <= bboxBounds.maxLat;
}

function firstCoordinateInBbox(coordinates, bboxBounds) {
  return coordinates.find((coordinate) => coordinateInBbox(coordinate, bboxBounds)) || coordinates[0] || null;
}

function representativeLonLatFromGeometry(geometry, bboxBounds = null) {
  const coordinates = allCoordinates(geometry);
  const normalized = coordinates
    .map((coord) => Array.isArray(coord) ? normalizeLonLatPair(coord[0], coord[1]) : null)
    .filter(Boolean);
  const direct = firstCoordinateInBbox(normalized, bboxBounds);
  if (direct) return direct;

  const projected = coordinates
    .map((coord) => Array.isArray(coord) ? utm33ToLonLat(coord[0], coord[1]) : null)
    .filter(Boolean);
  return firstCoordinateInBbox(projected, bboxBounds);
}

function accidentPointFeature(object, bboxBounds = null) {
  const geometry = objectGeometry(object);
  const coordinate = representativeLonLatFromGeometry(geometry, bboxBounds) || lonLatFromObjectFields(object);
  if (!coordinate || !coordinateInBbox(coordinate, bboxBounds)) return null;
  return {
    type: 'Feature',
...
function representativePointFeature(feature) {
  ...
  const coordinate = allCoordinates(feature.geometry)
    .find(isWgs84Coordinate);
...
function featuresForObject(object, layerType, options = {}) {
  if (layerType === 'accidents') {
    const accidentFeature = accidentPointFeature(object, options.bboxBounds || null);
...
function coordinateSummary(features = []) {
  const coordinates = features
    .flatMap((feature) => allCoordinates(feature.geometry))
    .map((coord) => Array.isArray(coord) ? coord.slice(0, 2).map(Number) : null)
    .filter((coord) => coord && coord.every(Number.isFinite));

  if (!coordinates.length) return { coordinateRange: null, derivedBbox: null };
  ...
  return {
    coordinateRange: { minLng, maxLng, minLat, maxLat },
    derivedBbox: [minLng, minLat, maxLng, maxLat],
  };
}

function validAccidentPointFeature(feature) {
  return feature?.type === 'Feature'
    && feature.geometry?.type === 'Point'
    && isWgs84Coordinate(feature.geometry.coordinates);
}

// A wide-bbox layer query (e.g. the accidents heatmap over a whole city) can
// easily hold more objects than fit on one NVDB page. Left unpaginated, the
// single page you get back isn't a random/spatial sample — NVDB tends to
// return objects in a stable, non-spatial order, so a big bbox can come back
// dominated by one heavily-catalogued stretch of road (e.g. an E-road) while
// everywhere else in the bbox silently has zero objects, which then reads as
// "the heatmap only shows the densest spot" once rendered. Follow
// metadata.neste.href to cover the whole bbox, capped so a huge bbox can't
// force unbounded fetch time.
const MAX_NVDB_OBJECTS_PER_QUERY = 2000;

async function getNvdbObjectsByParams({ type, params, cacheKey, fallbackPath }) {
  const config = NVDB_LAYER_TYPES[type];
  const startedAt = Date.now();
  let payload = await nvdbGet(`/vegobjekter/api/v4/vegobjekter/${config.objectTypeId}`, params, cacheKey);
  const objects = extractObjects(payload);
  let nextHref = payload?.metadata?.neste?.href || null;
  let pages = 1;
  let truncated = false;
  while (nextHref) {
    if (objects.length >= MAX_NVDB_OBJECTS_PER_QUERY) { truncated = true; break; }
    // Each page can itself retry/timeout for several seconds; without an
    // overall budget a wide bbox chained through a slow/degraded NVDB could
    // hold the request open for minutes even while staying under the
    // object-count cap. Stop paginating (keeping what's fetched so far, same
    // as a failed page below) once the whole call has run long enough.
    if (Date.now() - startedAt >= paginationBudgetMs()) {
      truncated = true;
      logNvdb('pagination_budget_exceeded', { type, fallbackPath, page: pages, elapsedMs: Date.now() - startedAt });
      break;
    }
    pages += 1;
    try {
      payload = await nvdbGetUrl(nextHref, `${cacheKey}:page${pages}`);
      objects.push(...extractObjects(payload));
      nextHref = payload?.metadata?.neste?.href || null;
    } catch (error) {
      // A later page failing shouldn't lose the objects already fetched —
      // return the partial set rather than throwing away a working first page.
      logNvdb('page_failed', { type, fallbackPath, page: pages, message: String(error?.message || '').slice(0, 240) });
      break;
    }
  }
  logNvdb('objects_returned', { type, fallbackPath, rawObjectCount: objects.length, pages, truncated });
  return objects;
}
...
export async function getNvdbLayerGeoJson({ type, bbox, zoom = null }) {
  const { objects, fallbackPath } = await getNvdbObjects({ type, bbox, antall: 250 });
  const bboxBounds = type === 'accidents' ? bboxValues(bbox) : null;
  const mappedFeatures = objects.flatMap((object) => featuresForObject(object, type, { bboxBounds }));
  const features = type === 'accidents' ? mappedFeatures.filter(validAccidentPointFeature) : mappedFeatures;
  const pointFeatureCount = features.filter((feature) => feature.geometry?.type === 'Point').length;
  const invalidGeometryCount = type === 'accidents'
    ? Math.max(0, objects.length - pointFeatureCount)
    : Math.max(0, objects.length - features.length);
  const { coordinateRange, derivedBbox } = coordinateSummary(features);
  const meta = {
    type,
    bbox,
    zoom,
  ...
  logNvdb('layer_geojson_completed', meta);
  return {
...
function summarizeAccidents({ objects, point, radiusMeters }) {
  ...
  const distances = objectsWithDistance.map(({ distance }) => distance);
