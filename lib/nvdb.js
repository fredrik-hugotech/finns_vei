...
function representativeLonLatFromGeometry(geometry) {
  const coordinate = allCoordinates(geometry).find(isWgs84Coordinate);
  if (coordinate) return coordinate.slice(0, 2);
  const projected = allCoordinates(geometry)
    .map((coord) => Array.isArray(coord) ? utm33ToLonLat(coord[0], coord[1]) : null)
    .find(Boolean);
  return projected || null;
}

function accidentPointFeature(object) {
  const geometry = objectGeometry(object);
  const coordinate = representativeLonLatFromGeometry(geometry) || lonLatFromObjectFields(object);
...
function featuresForObject(object, layerType) {
  if (layerType === 'accidents') {
    const accidentFeature = accidentPointFeature(object);
...
export async function getNvdbLayerGeoJson({ type, bbox, zoom = null }) {
  ...
  const mappedFeatures = objects.flatMap((object) => featuresForObject(object, type));
