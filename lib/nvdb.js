...
async function fetchJsonWithRetry(url) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    ...
    try {
      ...
      lastError = error;
      logNvdb('request_error', { url: url.toString(), attempt: attempt + 1, name: error?.name, code: error?.cause?.code || error?.code, message: String(error?.message || '').slice(0, 240), transient: isTransientNetworkError(error) });
...
export async function getRoadPosition({ lat, lng }) {
  ...
  logNvdb('position_summary', { lat, lng, matches: Array.isArray(matches) ? matches.length : 0, hasMatch: Boolean(match), roadReference: match?.vegsystemreferanse?.kortform || null });
  return match;
}
