...
export async function addTrelloCardAttachment(cardId, { url, name } = {}) {
  if (!cardId || !url || !process.env.TRELLO_API_KEY || !process.env.TRELLO_API_TOKEN) {
    logTrello('attachment_skipped_missing_env_card_or_url', { hasCardId: Boolean(cardId), hasUrl: Boolean(url), ...trelloEnvStatus() });
    return null;
  }
  logTrello('attachment_started', { cardId, name: name || null });
  const attachment = await trelloRequest(`/cards/${encodeURIComponent(cardId)}/attachments`, { url, name: name || 'Bilde' }, 'POST');
  logTrello('attachment_completed', { cardId, attachmentId: attachment?.id || null });
  return attachment;
}
...
export async function updateTrelloCardName(cardId, name) {
  if (!cardId || !name || !process.env.TRELLO_API_KEY || !process.env.TRELLO_API_TOKEN) {
    return null;
  }
  logTrello('rename_started', { cardId });
  const card = await trelloRequest(`/cards/${encodeURIComponent(cardId)}`, { name }, 'PUT');
  logTrello('rename_completed', { cardId });
  return card;
}

export async function getTrelloCardActions(cardId, { limit = 8 } = {}) {
  if (!cardId || !process.env.TRELLO_API_KEY || !process.env.TRELLO_API_TOKEN) {
    logTrello('actions_skipped_missing_env_or_card', { hasCardId: Boolean(cardId), ...trelloEnvStatus() });
    return [];
  }
  const actions = await trelloRequest(`/cards/${encodeURIComponent(cardId)}/actions`, {
    filter: 'commentCard,updateCard',
    limit: Math.max(1, Math.min(Number(limit) || 8, 10)),
  }, 'GET');
  return Array.isArray(actions) ? actions : [];
}

export async function addTrelloCardComment(cardId, text) {
  if (!cardId || !text || !process.env.TRELLO_API_KEY || !process.env.TRELLO_API_TOKEN) {
    logTrello('comment_skipped_missing_env_card_or_text', { hasCardId: Boolean(cardId), hasText: Boolean(text), ...trelloEnvStatus() });
    return null;
  }
  logTrello('comment_started', { cardId });
  const comment = await trelloRequest(`/cards/${encodeURIComponent(cardId)}/actions/comments`, { text }, 'POST');
  logTrello('comment_completed', { cardId, actionId: comment?.id || null });
  return comment;
}
