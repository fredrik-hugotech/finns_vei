...
// Instant single-report notification, sent right when a case is registered
// (see pages/api/report.js) — a lighter-weight sibling to the daily digest
// below, reusing the same reportCard() so both emails look consistent.
export function buildNewReportEmail({ report, baseUrl = '' } = {}) {
  const subject = `Finns Fairway – ny sak: ${report.category || 'Sak'}`;
  const adminUrl = baseUrl ? `${baseUrl}/backoffice/sak/${encodeURIComponent(report.id)}` : '';
        ...
        <tr><td style="padding:0 0 16px 0">
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background:${GREEN};border-radius:14px">
            <tr><td style="padding:22px 20px">
              ...
              <div style="color:#ffffff;font-size:22px;font-weight:800;margin-top:6px">Ny sak registrert</div>
            </td></tr>
          </table>
        </td></tr>
        ${reportCard(report, baseUrl)}
        <tr><td style="padding:8px 4px 0 4px;color:${MUTED};font-size:13px;line-height:1.5">
          ${adminUrl ? `<a href="${adminUrl}" style="color:${GREEN};font-weight:700;text-decoration:none">Åpne saken i backoffice</a><br>` : ''}
          Dette er en automatisk varsling per registrert sak. Meldinger fra barn er anonyme.
        </td></tr>
      </table>
    </td></tr>
  </table>
