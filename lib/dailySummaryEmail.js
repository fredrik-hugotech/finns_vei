...
// Defensive cap on how many report cards go into a single email — a
// high-volume day shouldn't be able to produce an oversized email that
// Resend (or the recipient's inbox) might reject or truncate. The subject
// and heading still reflect the true count; anything past the cap is called
// out in a closing line instead of being silently dropped.
const MAX_CARDS = 40;
...
export function buildDailySummaryEmail({ reports = [], baseUrl = '', dateLabel = '', windowHours = 24 } = {}) {
  ...
  const cards = shown.map((report) => reportCard(report, baseUrl)).join('');
    ...
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" role="presentation" style="max-width:600px;width:100%">
        ...
        <tr><td style="padding:8px 4px 0 4px;color:${MUTED};font-size:13px;line-height:1.5">
          ${overflow > 0 ? `<div style="margin-bottom:6px;font-weight:700;color:${INK}">… og ${overflow} til, se backoffice.</div>` : ''}
