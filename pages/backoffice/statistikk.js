...
// Internal, staff-only stats dashboard. Replaces the public /statistikk page
// that was removed on 2026-07-12 ("public stats aren't wanted; build internal
// stats separately"). Same aggregation logic as that page, but the data now
// comes from the admin-gated /api/backoffice/stats route and the framing is
// internal — no "these are open/public numbers" language.
...
// GeoJSON from /api/backoffice/stats is the same contact-info-stripped
// dataset the map uses (see report_public_geojson / lib/supabaseRest.js's
// stripPrivateProperties), so exporting it client-side adds no new privacy
// exposure — it's the same numbers, just downloadable instead of only
// chart-shaped.

// Columns kept in the CSV: straightforward, already-stripped scalar fields.
...
// All aggregation happens client-side over the GeoJSON from
// /api/backoffice/stats — no new server logic, just counting the features.
...
export default function BackofficeStatistikk() {
  ...
  useEffect(() => {
    ...
    fetch('/api/backoffice/stats')
      .then((r) => (r.status === 403
        ? Promise.reject(new Error('not-authed'))
        : (r.ok ? r.json() : Promise.reject(new Error('Kunne ikke hente data')))))
      .then((data) => {
        if (cancelled) return;
      ...
      .catch((e) => {
        if (!cancelled) setState({ status: e.message === 'not-authed' ? 'not-authed' : 'error', features: [] });
      });
  ...
  return (
    <>
      <Head>
        <title>Statistikk (internt) – Finns Fairway</title>
      ...
      <main className="page stats-page">
        <p className="dash2__hint" style={{ margin: 0 }}>
          Internt overblikk over innmeldinger — kategorier, status og utvikling over tid. Samme data som vises på
          kartet, oppsummert for stab.
        </p>
        ...
        {state.status === 'empty' && (
          <div className="admin-section stats-empty">
            ...
            <p className="comp-muted">
              Det er ikke registrert noen innmeldinger å vise statistikk for ennå. Kom tilbake senere.
            </p>
          </div>
        )}
          ...
          <>
            ...
            <section>
              ...
              <p className="stats-page__note">
                ...
                over). Ingen kontaktinfo følger med.
              </p>
            ...
            <p className="stats-page__foot comp-muted">
              Tallene oppdateres når du laster siden på nytt.
            </p>
          </>
        )}
      </main>
